...
function render(ctx) {
  const { entries, escapeHtml, formatPct, wrapTable, formatGamesAbbrev } = ctx;
  ...
  return `
    ...
    ${mainScores.length ? `<h2>At ${escapeHtml(mainBand)}</h2><p>${mainScores.map((s, i) => `${i === 0 ? '' : i === mainScores.length - 1 ? ' and ' : ', '}<strong>${escapeHtml(s.name)}</strong> scores ${formatPct(s.score)}%`).join('')} for Black, from real games in this rating band. The gap between the highest and lowest here is ${formatPct(mainScores[0].score - mainScores[mainScores.length - 1].score)} percentage points - worth knowing, but small enough that it shouldn&rsquo;t be the only reason to pick one over another.</p>` : ''}
