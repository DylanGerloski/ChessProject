...
function render(ctx) {
  const { entries, escapeHtml, wrapTable, formatGamesAbbrev } = ctx;
  ...
  const headerCells = picked.map((e) => `<th scope="col">${escapeHtml(e.model.name)}</th>`).join('');
  const rows = BANDS.map((band) => {
    const cells = picked
      .map((e) => {
        const b = e.model.bands.find((x) => x.band === band);
        return `<td>${b && b.enoughData ? `${b.scoreForSide}% <span class="rep-pct">(${formatGamesAbbrev(b.games)} games)</span>` : 'not enough games'}</td>`;
      })
      .join('');
  ...
  return `
    <p>1.e4 c5 (Sicilian), 1.e4 e6 (French), and 1.e4 c6 (Caro-Kann) are the three most-argued-about replies to 1.e4 for players who don't want to meet it symmetrically with 1...e5. Beginner forums debate which "scores best" constantly, usually without looking at any actual data. Here's what this site's own tracked games show.</p>
    ...
    ${mainScores.length ? `<h2>At ${escapeHtml(mainBand)}</h2><p>${mainScores.map((s, i) => `${i === 0 ? '' : i === mainScores.length - 1 ? ' and ' : ', '}<strong>${escapeHtml(s.name)}</strong> scores ${s.score}%`).join('')} for Black, from real games in this rating band. The gap between the highest and lowest here is ${Number((mainScores[0].score - mainScores[mainScores.length - 1].score).toFixed(1))} percentage points -- worth knowing, but small enough that it shouldn't be the only reason to pick one over another.</p>` : ''}

    <h2>What the data can't tell you</h2>
    <p>None of these numbers measure how much memorization each opening demands, or how sharp/quiet the resulting positions are -- both of which are usually the real deciding factor for a human choosing between them, and neither is something a database can score. The Sicilian is generally considered the most theory-heavy of the three and the Caro-Kann the least; that's established opening-theory consensus, not something measured on this page.</p>
