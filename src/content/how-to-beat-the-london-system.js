...
function render(ctx) {
  const { entries, escapeHtml, wrapTable } = ctx;
  ...
  const bestRows = byScore
    .slice(0, 3)
    .map(
      (m) => `<tr><td><a href="https://lichess.org/analysis/pgn/${encodeURIComponent(m.san)}">${escapeHtml(m.san)}</a></td><td>${m.games.toLocaleString()}</td><td>${m.playedPct}%</td><td>${m.blackScore}%</td></tr>`
    )
    .join('');
  ...
  const mistakesHtml = (model.mistakes || []).length
    ? `<ul>${model.mistakes
        .map(
          (m) => `<li class="callout"><strong>${escapeHtml(m.san)}</strong> is played in ${m.playedPct}% of games in this exact position, but scores only ${m.score}% for Black at ${escapeHtml(model.defaultBand)}${
            m.punishingReply
              ? ` -- White's most common answer, <strong>${escapeHtml(m.punishingReply.san)}</strong>, scores ${Number((m.punishingReply.winPct + m.punishingReply.drawPct / 2).toFixed(1))}% for White.`
              : '.'
          }</li>`
        )
        .join('')}</ul>`
  ...
  return `
    <p>The London System (1.d4 d5 2.Bf4) has a reputation as a low-theory, hard-to-punish opening for White, and the data mostly backs that up: across the games this site tracked, White scores ${mainBand.scoreForSide != null ? `${mainBand.scoreForSide}%` : 'a solid share'} at ${escapeHtml(model.defaultBand)} from ${mainBand.games.toLocaleString()} games. That doesn't mean every reply for Black is equally good.</p>

    <h2>What the data shows</h2>
    <p>Looking at the most common replies to 2.Bf4 in this position, the highest-scoring options for Black -- ranked by actual result, not by popularity -- are:</p>
    ...
    ${mostCommon ? `<p>For comparison, the single most commonly played reply here is <strong>${escapeHtml(mostCommon.san)}</strong> (${mostCommon.games.toLocaleString()} games) -- popular is not automatically the same as best-scoring, which is exactly why this table is sorted by score rather than by frequency.</p>` : ''}

    <h2>A mistake the data flags</h2>
    <p>Independent of any specific "trap," the numbers on this exact position highlight moves that are common but underperform:</p>
    ${mistakesHtml}
