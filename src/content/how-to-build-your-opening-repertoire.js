...
function render(ctx) {
  ...
  const sampleExampleHtml = cliff
    ? `<p>Here&rsquo;s the cliff inside a single position from this build. In the <a href="${escapeHtml(cliff.openingSlug)}.html">${escapeHtml(cliff.openingName)}</a> position at ${escapeHtml(cliff.band)}, the most-played reply, <strong>${escapeHtml(cliff.most.san)}</strong>, has ${cliff.most.games.toLocaleString()} games behind its ${formatPct(cliff.most.winPct)}% win rate; the least-played candidate on the same menu, <strong>${escapeHtml(cliff.least.san)}</strong>, has ${cliff.least.games.toLocaleString()} &mdash; about ${Math.round(cliff.ratio).toLocaleString()}&times; fewer. Here both samples are still large enough that the &plusmn; stays small (${ciText(cliff.most.winCI)} and ${ciText(cliff.least.winCI)} points respectively) &mdash; but the wobble scales as one over the square root of the sample, so every 100&times; drop in games multiplies the &plusmn; by 10. Follow a rare move two or three times in a row, where positions often carry a few thousand games rather than a few million, and that same arithmetic puts the &plusmn; past a full point &mdash; wider than most of the score gaps you&rsquo;d be using the number to judge.</p>`
  ...
  const trapHtml = trap
    ? `<p>Here&rsquo;s a live example from this build. In the <a href="${escapeHtml(trap.openingSlug)}.html">${escapeHtml(trap.openingName)}</a> position at ${escapeHtml(trap.band)}, the most-played reply is <strong>${escapeHtml(trap.mostPlayed.san)}</strong> (${formatGamesAbbrev(trap.mostPlayed.games)} games), scoring ${formatPct(trap.mostPlayed.score)}% for ${escapeHtml(trap.side)} &mdash; but <strong>${escapeHtml(trap.bestScoring.san)}</strong> (${formatGamesAbbrev(trap.bestScoring.games)} games) scores ${formatPct(trap.bestScoring.score)}%, a ${formatPct(trap.gap)}-point difference that comfortably clears both moves&rsquo; &plusmn; ranges. Players keep choosing the first move anyway, mostly because it&rsquo;s the move they&rsquo;ve seen. Popularity measures familiarity, not quality.</p>`
    : '<p class="empty-note">In this build, no tracked position had a most-played reply that measurably underperformed the best-scoring one by more than the data&rsquo;s own uncertainty &mdash; a rarity worth noting in itself.</p>';
  ...
  return `
    <p>An opening repertoire sounds like something only titled players need. In practice it just means: a prepared answer to 1.e4, a prepared answer to 1.d4, and one or two choices of your own with White &mdash; a handful of positions you understand, instead of a different surprise every game. This page is a method for choosing that handful using real game data, with every example below computed from this site&rsquo;s own tracked positions rather than asserted.</p>

    <h2>Step 1: start from a shortlist, not from everything</h2>
    <p>There are hundreds of named openings; you need about four. The ${shortlist.length || 'ten'} tracked on this site are a deliberately conventional starting menu &mdash; mainstream, sound, and common enough at club level that there&rsquo;s real data on each:</p>
    ...
    <p>Pick candidates by the positions you like being in, then use the numbers to referee between them &mdash; not the other way around. A 2-point score edge will not save you in a position you hate playing.</p>

    <h2>Step 2: weigh the sample size before the win rate</h2>
    <p>The single most common way to misread Opening Explorer stats is treating every percentage as equally solid. A score from 500 games can easily be several points away from the &ldquo;true&rdquo; number; a score from half a million games barely moves. The &plusmn; figures on this site&rsquo;s tables are exactly this: the statistical wobble left in each percentage given how many games produced it. At the mainline crossroads in the table above, the samples run to tens of millions and the wobble is negligible &mdash; those score columns you can simply trust. The trap is that sample sizes fall off a cliff the moment you leave the main road, while the percentages keep looking just as confident.</p>
    ${sampleExampleHtml}
    <p>The practical rule: when two moves&rsquo; scores are closer together than their &plusmn; ranges are wide, the data does not actually say one is better &mdash; and choosing between them on &ldquo;53.1 beats 52.4&rdquo; is reading noise as signal.</p>

    <h2>Step 3: don&rsquo;t confuse popular with good</h2>
    <p>The biggest number on any explorer page is the games count next to the most common move &mdash; and it quietly suggests that move is the answer. Sometimes it is. Often the second- or third-most-played move scores meaningfully better, because popularity lags what actually works at your level.</p>
    ${trapHtml}

    <h2>Step 4: deviate from theory where the data says to</h2>
    <p>&ldquo;Theory&rdquo; is the record of what works when both sides know what they&rsquo;re doing &mdash; which is not a description of club chess. ${mistakeCount > 0
      ...
      : `This site applies a deliberately strict test for calling a common move a measurable underperformer (the gap has to clear the statistical uncertainty on both sides, among evenly-matched opponents) &mdash; in the current build&rsquo;s data, no tracked position clears that bar, which is itself a useful calibration: most &ldquo;that move is a mistake&rdquo; claims you&rsquo;ll hear are not backed by a gap the data can actually distinguish.`}
    Two consequences for your repertoire: a slightly offbeat line that scores well at your band is a legitimate choice, whatever a database of master games thinks of it &mdash; and the moves worth memorizing first are the ones that punish what your opponents actually play, not what theory says they should play. <a href="most-common-opening-mistakes-1600-1800.html">The most common opening mistakes at 1600-1800 &rarr;</a> applies this same test position by position.</p>

    <h2>Step 5: keep it small, then test it against real games</h2>
    <p>Four openings, understood, beat twelve memorized. Once you&rsquo;ve picked, the maintenance loop is short: play the lines, notice where you keep landing in trouble, and check that exact position&rsquo;s numbers at your own rating band &mdash; not at master level &mdash; before deciding whether the problem is the opening or the middlegame that followed it. For a ready-made starting point by rating and color, see the <a href="repertoire.html">repertoire explorer &rarr;</a>; for the full band-by-band numbers behind every opening above, the <a href="openings.html">openings comparison &rarr;</a>.</p>
  `;
}
