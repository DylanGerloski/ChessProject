...
function render(ctx) {
  ...
  return `
    <p>&ldquo;Don&rsquo;t study openings, just learn tactics and endgames&rdquo; is standard advice for players under 1500, and it&rsquo;s aimed at how you should spend study time &mdash; not really a claim about whether openings themselves perform differently by rating. This page checks a version of that second question that actually is measurable: for the exact same opening line, how much does the featured side&rsquo;s score change between rating bands?</p>
    ...
    <p>This measures whether the SAME position scores differently by rating band &mdash; it does not measure whether studying openings improves your results faster than studying anything else, which would need a completely different kind of study (tracking individual players&rsquo; progress over time), not a snapshot of aggregate game data.</p>
