...
test('buildStatic also writes sitemap.xml (listing exactly the emitted .html pages) and robots.txt pointing at it', () =>
  withTempDist(async () => {
    ...
    // WS-1 placeholder pages (still noindex) -- repertoire-builder.html
    // shipped for real (WS-1 W1a) and is no longer excluded here.
    ...
    assert.ok(locMatches.includes('https://repertoire-builder.com/repertoire-builder.html'), 'the WS-1 repertoire builder shipped for real (W1a) and must now be in the sitemap');
