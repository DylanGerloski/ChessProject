...
test('addGame + finalize: every position along the path gets a reached count; every edge its own move count', () => {
  ...
  // ply 0 (start), 1 (after e4), 2 (after e5) are all <= ROOT_MAX_PLY (3) -> root.
  ...
  assert.equal(families.size, 0); // whole 3-ply path is within ROOT_MAX_PLY (3) -> all root
});
...
test('finalize: a deep position (ply > ROOT_MAX_PLY) shards under its family, not root', () => {
  const builder = new AggregateBuilder();
  // 8 plies deep -> the final position is ply 8, beyond ROOT_MAX_PLY (3).
...
test('finalize: pathIndex maps a ply<=ROOT_MAX_PLY uci path to its posKey', () => {
  const builder = new AggregateBuilder();
